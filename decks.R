...
decks<-decks%>%#priority 7: Tournament cEDH
  add_row(deck="Dancing Goblins",commander="Gallia",power="9",played=3,priority="7",W=F,U=F,B=F,R=T,G=T)%>% 
  add_row(deck="Doomed by the Obscura",commander="Toluz",power="10",played=3,priority="7",W=T,U=T,B=T,R=F,G=F)%>% 
  add_row(deck="Metallic Shark",commander="Shabraz & Brallin",power="9",played=3,priority="7",W=F,U=T,B=T,R=T,G=F)%>%  
  add_row(deck="Sultai Mean Girls",commander="Sidisi",power="9",played=2,priority="7",W=F,U=T,B=T,R=F,G=T)%>%
  add_row(deck="The Weatherlight is Under Construction",commander="Jhoira",power="9",played=4,priority="7",W=F,U=T,B=F,R=T,G=F)%>%
  ...
  add_row(deck="Ethics Violations",commander="Volrath",power="6",played=2,priority="5",W=F,U=T,B=T,R=F,G=T)%>%
  add_row(deck="Failed Parley",commander="Phabine",power="5",played=5,priority="5",W=F,U=T,B=T,R=F,G=T)%>%
  add_row(deck="Gifts to the Table",commander="Parnesse",power="6",played=8,priority="2",W=F,U=T,B=T,R=T,G=F)%>%
  add_row(deck="Graveyard of Nightmares",commander="Beamtown Bullies",power="5",played=1,priority="5",W=F,U=F,B=T,R=T,G=T)%>%
  add_row(deck="Hellrider's Pride",commander="Zevlor",power="5",played=3,priority="5",W=F,U=T,B=T,R=T,G=F)%>%
  add_row(deck="Keep People Humble",commander="Ivy",power="5",played=3,priority="5",W=F,U=T,B=F,R=F,G=T) %>% 
  add_row(deck="Let Me Borrow This",commander="Yasova",power="5",played=4,priority="5",W=F,U=T,B=T,R=T,G=F)%>%
  add_row(deck="Let's All Work Together",commander="Sen Triplets",power="6",played=6,priority="5",W=T,U=T,B=T,R=F,G=F)%>%  
  add_row(deck="Party of Two",commander="Minthara",power="5",played=0,priority="5",W=T,U=F,B=T,R=F,G=F)%>%
  add_row(deck="Point Mutation",commander="Otrimi",power="5",played=3,priority="5",W=T,U=T,B=F,R=F,G=T) %>%
  ...
  add_row(deck="Weather The Storm",commander="Trelassara",power="5",played=0,priority="2",W=T,U=F,B=F,R=F,G=T)%>%
  ...
  add_row(deck="Baby Bears",commander="Ruxa",power="5",played=0,priority="1",W=F,U=F,B=F,R=F,G=T)%>%
  add_row(deck="Bad Girl",commander="Auntie Blythe",power="5",played=0,priority="1",W=F,U=F,B=F,R=T,G=F)%>%
  ...
  add_row(deck="Defending Fblthp",commander="Fblthp",power="4",played=0,priority="1",W=F,U=T,B=F,R=F,G=F)%>%
  add_row(deck="Descend into Madness",commander="Anhelo",power="4",played=0,priority="1",W=F,U=T,B=T,R=T,G=F)%>%
  ...
  add_row(deck="Dynaheir's Honour Guard",commander="Dynaheir",power="5",played=0,priority="1",W=T,U=T,B=F,R=T,G=F)%>%   
  ...
  add_row(deck="Illusions of Grandeur",commander="Zur",power="5",played=0,priority="1",W=T,U=T,B=T,R=F,G=F)%>%
  ...
  add_row(deck="Just a Glimpse",commander="Kenessos",power="5",played=0,priority="1",W=F,U=T,B=F,R=F,G=T)%>%
  ...
  add_row(deck="Post-life Shenanigans",commander="Agrus Kos",power="5",played=0,priority="1",W=T,U=F,B=F,R=T,G=F)%>%
  add_row(deck="Poor Little Goblins",commander="Gut",power="4",played=0,priority="1",W=F,U=F,B=T,R=T,G=F)%>% 
  ...
  add_row(deck="Prismari for Life",commander="Galazeth Prismari",power="5",played=0,priority="1",W=F,U=T,B=F,R=T,G=F)%>%
  add_row(deck="Pyramid of Zombies",commander="Temmet",power="5",played=0,priority="1",W=T,U=T,B=F,R=F,G=F)%>%
  add_row(deck="Remember, Remember",commander="Geist of Saint Traft",power="5",played=0,priority="1",W=F,U=T,B=T,R=F,G=T) %>%
  ...
  add_row(deck="Royal Snowball",commander="Queen Marchesa",power="9",played=0,priority="1",W=T,U=F,B=T,R=T,G=F)%>%
  add_row(deck="Sands of the Forgotten",commander="Hazezon",power="5",played=0,priority="1",W=T,U=F,B=F,R=T,G=T) %>%
  add_row(deck="Second Breakfast",commander="Piru",power="5",played=0,priority="1",W=T,U=F,B=T,R=T,G=F) %>%
  ...
  add_row(deck="Smash!",commander="Kibo",power="6",played=0,priority="1",W=F,U=F,B=F,R=T,G=T)%>%
  ...
  add_row(deck="The Stacl is Your Wonderland",commander="Neera",power="5",played=0,priority="1",W=F,U=T,B=F,R=T,G=F)%>%
  add_row(deck="The Saga of Abdel",commander="Abdel & Cloakwood Hermit",power="5",played=0,priority="1",W=T,U=F,B=F,R=F,G=T)%>%  
  add_row(deck="Tron Union",commander="Self-Assembler",power="5",played=0,priority="1",W=F,U=F,B=F,R=F,G=F)%>%  
  add_row(deck="True Love",commander="Regna & Krav",power="5",played=0,priority="1",W=T,U=F,B=T,R=F,G=F)%>%  
  add_row(deck="Trust the Process",commander="Ukkima & Cazur",power="9",played=0,priority="1",W=F,U=T,B=T,R=F,G=T)%>%
  add_row(deck="The Secret Life of Hell",commander="Raphael",power="5",played=0,priority="1",W=F,U=F,B=T,R=T,G=F)%>%  
  add_row(deck="Undercity Expedition",commander="Tocasia",power="5",played=0,priority="1",W=T,U=F,B=T,R=F,G=T)%>%  
...
#deck stats 
games<-decks%>%filter(priority!="1"&priority!="0")%>%ggplot(aes(played,deck,fill=played))+
        geom_bar(stat="identity")+
